...
const { validatePlainText } = require('../utils/validators');
...
exports.postNewReview = async (req, res) => {
  try {
    ...
    const contentValidation = validatePlainText(content, 'Рецензия', { required: true, min: 10, max: 5000 });
  ...
  try {
    ...
    if (errors.length > 0) {
      ...
      return res.render('reviews/edit', {
        ...
        content: contentValidation.value || review.content
      });
    }
