...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderPagination = (container) => {
    ...
    const addEllipsis = (target, where) => {
      const tag = target.tagName.toLowerCase() === 'a' ? 'a' : 'span';
      const ellipsis = document.createElement(tag);
      ellipsis.className = `${target.className.replace(/\bactive\b/g, '').trim()} pagination-ellipsis`;
      ellipsis.textContent = '…';
      if (ellipsis.tagName.toLowerCase() === 'a') ellipsis.removeAttribute('href');
      if (where === 'before') {
        target.parentNode.insertBefore(ellipsis, target);
      } else {
        target.parentNode.insertBefore(ellipsis, target.nextSibling);
      }
    };

    if (Number(firstVisible.textContent.trim()) > 1) {
      addEllipsis(firstVisible, 'before');
    }

    if (Number(lastVisible.textContent.trim()) < totalPages) {
      addEllipsis(lastVisible, 'after');
    }
  };
